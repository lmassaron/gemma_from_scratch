...
def create_train_state(rng, config, learning_rate_fn, sequence_length):
    ...
    # Optimizer
    tx = optax.adamw(
        learning_rate=learning_rate_fn, b1=0.9, b2=0.95, eps=1e-9, weight_decay=0.1
    )
...
def train_step(state, batch, accumulation_steps):
    ...
    final_accum_grads, losses = jax.lax.scan(
        accumulate_grads, init_accum_grads, (inputs, targets)
    )
...
def main(args):
    ...
    best_model_params_path = os.path.join(
        models_dir, f"best_model_jax_{timestamp}.params"
    )
    ...
    if num_optimizer_steps <= warmup_optimizer_steps:
        print(
            f"Warning: max_iters ({args.max_iters}) is less than or equal to warmup_steps ({args.warmup_steps}). Adjusting schedule to warmup-only."
        )
        warmup_optimizer_steps = (
            num_optimizer_steps - 1
        )  # Ensure strictly less than total if possible, or just accept short warmup
    ...
    state = create_train_state(
        init_rng, GEMMA3_CONFIG_CUSTOM, lr_schedule, args.sequence_length
    )

    print(
        f"Model initialized. Params count: {sum(x.size for x in jax.tree_util.tree_leaves(state.params))}"
    )
    ...
    for update_step in pbar:
        ...
        state, loss = train_step(
            state, (inputs, targets), args.gradient_accumulation_steps
        )
    ...
    final_model_params_path = os.path.join(
        models_dir, f"final_model_jax_{timestamp}.params"
    )
