...
def load_pytorch_weights(path, jax_params):
    ...
    def set_param(jax_path, torch_names, transpose=False):
        ...
        if not found_name:
            print(f"Warning: None of {torch_names} found in state_dict. Keeping random initialization for {jax_path[-1]}")
            return
        ...
        if w.shape != target_shape:
             print(f"Shape mismatch for {found_name}: torch {w.shape} vs jax {target_shape}")
             return
    ...
    for i in range(n_layers):
        ...
        set_param([jax_block, 'att', 'W_query', 'kernel'], 
                  [f'model.layers.{i}.self_attn.q_proj.weight', f'blocks.{i}.att.W_query.weight'], transpose=True)
        set_param([jax_block, 'att', 'W_key', 'kernel'], 
                  [f'model.layers.{i}.self_attn.k_proj.weight', f'blocks.{i}.att.W_key.weight'], transpose=True)
        set_param([jax_block, 'att', 'W_value', 'kernel'], 
                  [f'model.layers.{i}.self_attn.v_proj.weight', f'blocks.{i}.att.W_value.weight'], transpose=True)
        set_param([jax_block, 'att', 'out_proj', 'kernel'], 
                  [f'model.layers.{i}.self_attn.o_proj.weight', f'blocks.{i}.att.out_proj.weight'], transpose=True)
        
        # Norms inside Attention (QK Norm)
        set_param([jax_block, 'att', 'q_norm_layer', 'scale'], 
                  [f'model.layers.{i}.self_attn.q_norm.weight', f'blocks.{i}.att.q_norm.scale'])
        set_param([jax_block, 'att', 'k_norm_layer', 'scale'], 
                  [f'model.layers.{i}.self_attn.k_norm.weight', f'blocks.{i}.att.k_norm.scale'])

        # MLP
        # HF: gate_proj, up_proj, down_proj
        # Scratch: fc1 (gate), fc2 (up), fc3 (down)
        set_param([jax_block, 'ff', 'fc1', 'kernel'], 
                  [f'model.layers.{i}.mlp.gate_proj.weight', f'blocks.{i}.ff.fc1.weight'], transpose=True)
        set_param([jax_block, 'ff', 'fc2', 'kernel'], 
                  [f'model.layers.{i}.mlp.up_proj.weight', f'blocks.{i}.ff.fc2.weight'], transpose=True)
        set_param([jax_block, 'ff', 'fc3', 'kernel'], 
                  [f'model.layers.{i}.mlp.down_proj.weight', f'blocks.{i}.ff.fc3.weight'], transpose=True)

        # LayerNorms
        # HF: weight, Scratch: scale
        set_param([jax_block, 'input_layernorm', 'scale'], 
                  [f'model.layers.{i}.input_layernorm.weight', f'blocks.{i}.input_layernorm.scale'])
        set_param([jax_block, 'post_attention_layernorm', 'scale'], 
                  [f'model.layers.{i}.post_attention_layernorm.weight', f'blocks.{i}.post_attention_layernorm.scale'])
        set_param([jax_block, 'pre_feedforward_layernorm', 'scale'], 
                  [f'model.layers.{i}.pre_feedforward_layernorm.weight', f'blocks.{i}.pre_feedforward_layernorm.scale'])
        set_param([jax_block, 'post_feedforward_layernorm', 'scale'], 
                  [f'model.layers.{i}.post_feedforward_layernorm.weight', f'blocks.{i}.post_feedforward_layernorm.scale'])
    ...
    if has_head_weight:
        set_param(['out_head', 'kernel'], ['lm_head.weight', 'out_head.weight'], transpose=True)
    else:
        # Weight tying
        print("Weight tying detected (output head weights missing). Sharing embeddings.")
