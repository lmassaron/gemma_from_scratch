...
def generate_report(model):
    ...
    for i, block in enumerate(model.blocks):
        ...
        attention_params = q_proj_params + k_proj_params + v_proj_params + out_proj_params
        ...
        post_attention_layernorm_params = count_parameters(block.post_attention_layernorm)
        post_attention_layernorm_size = get_module_param_shape(block.post_attention_layernorm, 'scale')
        ...
        ln_attention_params = input_layernorm_params + post_attention_layernorm_params + q_norm_params + k_norm_params
        ...
        # Layer Norms (MLP)
        pre_feedforward_layernorm_params = count_parameters(block.pre_feedforward_layernorm)
        pre_feedforward_layernorm_size = get_module_param_shape(block.pre_feedforward_layernorm, 'scale')
        post_feedforward_layernorm_params = count_parameters(block.post_feedforward_layernorm)
        post_feedforward_layernorm_size = get_module_param_shape(block.post_feedforward_layernorm, 'scale')

        ln_mlp_params = pre_feedforward_layernorm_params + post_feedforward_layernorm_params

        block_total = attention_params + ln_attention_params + mlp_params + ln_mlp_params
        ...
        report_lines.append("-" * 97)
        add_line(f"input_layernorm_{i}", input_layernorm_params, input_layernorm_size, level=1)
        add_line(f"post_attention_layernorm_{i}", post_attention_layernorm_params, post_attention_layernorm_size, level=1)
        ...
        report_lines.append("-" * 97)
        add_line(f"pre_feedforward_layernorm_{i}", pre_feedforward_layernorm_params, pre_feedforward_layernorm_size, level=1)
        add_line(f"post_feedforward_layernorm_{i}", post_feedforward_layernorm_params, post_feedforward_layernorm_size, level=1)
